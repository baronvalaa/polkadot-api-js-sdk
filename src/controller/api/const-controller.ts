...
export class ConstController implements IGroupableController {
	...
	handleGetBabeEpochDuration = async (req: Request, res: Response, next: Next) => {
		try {
			...
			res.send(200, this._api.consts.babe.epochDuration.toHuman());
			next();
		} catch (err) {
			console.error(err);
			next(err);
	...
	handleGetBalancesExistentialDeposit = async (req: Request, res: Response, next: Next) => {
		try {
			await this._api.isReady;
			const result = this._api.consts.balances.existentialDeposit.toHuman();
			res.send(200, result);
			next();
		} catch (err) {
			console.error(err);
	...
	handleGetTransactionPaymentTransactionByteFee = async (req: Request, res: Response, next: Next) => {
		try {
			await this._api.isReady;
			const result = this._api.consts.transactionPayment.transactionByteFee.toHuman();
			res.send(200, result);
			next();
		} catch (err) {
			console.error(err);
